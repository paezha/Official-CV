...
The funding total of all projects that I have been part of is `r paste0("CAD ", prettyNum(sum(research_funding$Amount_num), big.mark = ",", digits = 4))`. Of this amount, `r paste0("CAD ", prettyNum(sum(research_funding$Amount_num[research_funding$Purpose == "Research"]), big.mark = ",", digits = 4))` has been research grants from various organizations, `r paste0("CAD ", prettyNum(sum(research_funding$Amount_num[research_funding$Purpose == "Teaching"]), big.mark = ",", digits = 4))` has been for pedagogical research, and `r paste0("CAD ", prettyNum(sum(research_funding$Amount_num[research_funding$Purpose == "Travel"]), big.mark = ",", digits = 4))` has been to support event participation.
...
```{r research-funding-plot, out.width="0.8\\linewidth", fig.cap="\\label{fig:research-funding-share}My share of research funds by source"}
...
The impact of my work can also be appreciated by the breadth of the fields in which it is cited. According to Web of Science (Fig. \ref{fig:citations-by-field}), my research is cited by scholars in numerous disciplines, notably Transportation, Geography, Economics, Environmental Studies and Science, Computer Science, and Civil Engineering. My publications have been cited by researchers in more than a 100 countries; as seen in Fig. \ref{fig:citations-by-country}, most citations to my work are by researchers in the United States, China, Canada, England, and Spain.
...
```{r citations-field, out.width="0.9\\linewidth", fig.cap="\\label{fig:citations-by-field}Fields where my research is cited according to Web of Science"}
knitr::include_graphics(glue(here(), "/assets/visualization_fields_2024_05_13.png"))
```

```{r citations-country, out.width="0.9\\linewidth", fig.cap="\\label{fig:citations-by-country}Countries where my research is cited according to Web of Science"}
knitr::include_graphics(glue(here(), "/assets/visualization_countries_2024_05_13.png"))
```
...
key_research_themes <- ggraph(graph, layout = "fr") +
...
intl_collabs <- ggraph(graph, layout = "linear", circular = TRUE) +
  geom_edge_fan(aes(alpha = weight)) +
...
```{r key-topics-plot, out.width="0.8\\linewidth", fig.cap="\\label{fig:key-topics}Key topics in my peer-reviewed publications"}
key_research_themes
```

```{r intl-colabs-plot, out.width="0.8\\linewidth", fig.cap="\\label{fig:intl-colabs}International collaborations"}
intl_collabs
```
...
talks <- read_csv(paste0(here(),"/assets/talks.csv")) |>
  ...
  detailed_entries(what = Organization, 
                   ...
                   .protect = FALSE)
```
## Lifetime talks, seminars, workshops, panels
